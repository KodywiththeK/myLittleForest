      ...
      {chatRooms.length !== 0 ? (
        chatRooms.map((room, index) => (
          ...
          >
            <div className="absolute inset-0 flex flex-col justify-between rounded-2xl bg-midNavy p-6 text-white">
              ...
              <BsThreeDots className="absolute top-5 right-5 text-[22px]" />
                ...
                <div className="flex w-full">
                  <div className="relative h-[90px] w-[130px] drop-shadow-2xl">
                    {room.randomProfiles.map((user, idx) => (
                      <img
                        key={idx}
                        src={user.pic}
                        className={`${user.position} absolute h-[60px] w-[60px] rounded-full object-cover`}
                      />
                    ))}
                  </div>
                  <div className="flex min-w-[50%] flex-col">
                    {room.randomProfiles.map((user, idx) => (
                      <div
                        key={idx}
                        className="my-1 ml-2 flex text-[20px] font-semibold"
                      >
        ...
        <div className="relative mb-5 flex min-h-[280px] w-full min-w-[100vw] flex-col">
          <div className="absolute inset-x-4 inset-y-0 flex flex-col items-center justify-center rounded-2xl bg-midNavy p-6 text-white">
            <div className="text-[19px] font-semibold leading-6">
              검색된 채팅방이 없습니다.
            </div>
          </div>
        </div>
